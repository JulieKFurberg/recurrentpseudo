...
#---------------------------------------------------------------#
## Fix 9 subjects ending with events -> treat as cens
# with(bladderlast, table(status))
bladdersub$lastobs <- rep(0, nrow(bladdersub))
bladdersub[!duplicated(bladdersub$id, fromLast =T),]$lastobs <- 1
nrow(subset(bladdersub, lastobs == 1))

blad2 <- bladdersub[bladdersub$lastobs == 1 & bladdersub$status == 1,]

blad2$status <- 0
blad2$start <- blad2$stop
blad2$stop <- blad2$stop +1
blad2$status3 <- 0

blad2

with(subset(bladdersub, lastobs == 1), table(status))

# Add extra censorings
bladdersub2 <- rbind(bladdersub, blad2)



# Pseudo-observations
pseudo_bladder_3d <- pseudo.threedim(tstart = bladdersub2$start,
                                     tstop = bladdersub2$stop,
                                     status = bladdersub2$status3,
                                     id = bladdersub2$id,
                                     deathtype = bladdersub2$deathtype,
                                     covar_names = "Z",
                                     tk = c(30), 
                                     data = bladdersub2)
head(pseudo_bladder_3d$outdata_long)
...
# lad os prøve at fitte det direkte med mets
last <- subset(bladdersub, lastobs == 1)
require(mets)
